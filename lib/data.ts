...
export const getVillageInfo = unstable_cache(
  async () => {
  ...
  ['village-info'],
  { revalidate: 60, tags: ['village-info'] }
);

export const getDemographicStats = unstable_cache(
  async () => {
  ...
  ['demographics'],
  { revalidate: 60, tags: ['demographics'] }
);

export const getOfficials = unstable_cache(
  async () => {
    const { data, error } = await supabase
      .from("officials")
      .select("*")
